...
public class CloudinaryService {
    ...
    public Map uploadImage(MultipartFile file) throws IOException {
        return cloudinary.uploader().upload(file.getBytes(),
                ObjectUtils.asMap(
                        "resource_type", "auto",
                        "folder", "music_app_images" // Tên thư mục trên Cloudinary
                ));
    }
    public Map uploadWallpaper(MultipartFile file) throws IOException {
        return cloudinary.uploader().upload(file.getBytes(),
                ObjectUtils.asMap(
                        "resource_type", "auto",
                        "folder", "music_app_wallpapers" // Tên thư mục trên Cloudinary
                ));
    }
    ...
    public Map deleteWallpaper(String publicId) throws IOException {
        return cloudinary.uploader().destroy(publicId,
                ObjectUtils.asMap("resource_type", "image"));
    }
